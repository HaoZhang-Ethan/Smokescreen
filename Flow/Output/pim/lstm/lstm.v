...
//FORGET GATE
  vecmat_mul_x #(`varraysize,`INPUT_DEPTH) f_gatex(.clk(clk),.reset(rst),.data(x_in),.tmp(macout_fx));
  vecmat_mul_h #(`uarraysize,`ARRAY_DEPTH) f_gateh(.clk(clk),.reset(rst),.data(h_in),.tmp(macout_fh));
...
//INPUT GATE
  vecmat_mul_x #(`varraysize,`INPUT_DEPTH) i_gatex(.clk(clk),.reset(rst),.data(x_in),.tmp(macout_ix));
  vecmat_mul_h #(`uarraysize,`ARRAY_DEPTH) i_gateh(.clk(clk),.reset(rst),.data(h_in),.tmp(macout_ih));
...
//OUTPUT GATE
  vecmat_mul_x #(`varraysize,`INPUT_DEPTH) o_gatex(.clk(clk),.reset(rst),.data(x_in),.tmp(macout_ox));
  vecmat_mul_h #(`uarraysize,`ARRAY_DEPTH) o_gateh(.clk(clk),.reset(rst),.data(h_in),.tmp(macout_oh));
...
//CELL STATE GATE
  vecmat_mul_x #(`varraysize,`INPUT_DEPTH) c_gatex(.clk(clk),.reset(rst),.data(x_in),.tmp(macout_cx));
  vecmat_mul_h #(`uarraysize,`ARRAY_DEPTH) c_gateh(.clk(clk),.reset(rst),.data(h_in),.tmp(macout_ch));
...
module vecmat_mul_h #(parameter uarraysize=1024,vectwidth=64) //,matsize=64)   // varraysize=1024 vectwidth=64,matsize=4096
(
 input clk,reset,
...
// wire overflow [vectwidth-1:0];  
 ...
 endgenerate*/
	...
	conv_top #(.INPUT_SIZE(`uarraysize/`DATA_WIDTH), .INPUT_P(`DATA_WIDTH), .DEPTH(1), .ADC_P(8), .OUT_P(`DATA_WIDTH)) PIM(
		.clk(clk), 
		.rst(rst),
